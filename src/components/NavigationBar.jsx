...
const NavigationBar = ({ active, setActive }) => {
  ...
  return (
    <header className=" z-[100] fixed sm:top-4 w-full h-full sm:w-auto sm:h-auto">
      <button
        onClick={() => showNav(!nav)}
        className={`fixed hover:text-gray-400 transition top-2 right-2 text-[2.25rem] sm:hidden`}
      >
        {nav
          ? React.createElement(IoIosCloseCircleOutline)
          : React.createElement(RxHamburgerMenu)}
      </button>
      <nav
        className={`bg-white/95 sm:px-7 sm:py-5 w-full h-full sm:rounded-full  sm:block ${
          nav ? "block" : "hidden"
        }`}
      >
